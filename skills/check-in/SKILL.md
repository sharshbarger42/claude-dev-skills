...
!`for repo in $HOME/gitea-repos/*/; do name=$(basename "$repo"); commits=$(cd "$repo" 2>/dev/null && git log --oneline --since="midnight" 2>/dev/null); [ -n "$commits" ] && echo "=== $name ===" && echo "$commits"; cd - >/dev/null; done`

### Uncommitted work
!`for repo in $HOME/gitea-repos/*/; do name=$(basename "$repo"); cd "$repo" 2>/dev/null && st=$(git status --porcelain 2>/dev/null) && [ -n "$st" ] && echo "=== $name ===" && echo "$st"; cd - >/dev/null; done`
